...
//
var acadeReference : AnimationReference!
  func setup(sceneSize: CGSize) {
    ...
    acadeButton = childNode(withName: "button-acade") as! TwoPaneButton
    ...
    acadeReference = AnimationReference(zeroPosition: acadeButton.position.x, offscreenLeft: -size.width - acadeButton.position.x, offscreenRight: size.width)
    ...
    acadeButton.addTarget(self, selector: #selector(acadeButtonClicked), forControlEvents: .TouchUpInside)
    
    
    }
    func acadeButtonClicked(_ sender : TwoPaneButton) {
        if let menu = menuNavigation {
            selectedButton = acadeButton
  ...
  func navigateInFromLeft(duration: TimeInterval) {
    ...
    if let button = selectedButton {
      if singlePlayerButton.isEqual(to: button) {
        ...
        acadeButton.moveTo(x: acadeReference.zeroPosition, duration: duration)
      ...
      } else if acadeButton.isEqual(to: button) {
        ...
        tutorialButton.moveTo(x: tutorialReference.zeroPosition, duration: duration, delay: delay * 2)
      }  else if multiPlayerButton.isEqual(to: button) {
        multiPlayerButton.moveTo(x: multiPlayerReference.zeroPosition, duration: duration)
        singlePlayerButton.moveTo(x: singlePlayerReference.zeroPosition, duration: duration, delay: delay)
        acadeButton.moveTo(x: acadeReference.zeroPosition, duration: duration)
  ...
  func navigateOutToLeft(duration: TimeInterval) {
    ...
    if let button = selectedButton {
        if singlePlayerButton.isEqual(to: button) {
            ...
            acadeButton.moveTo(x: acadeReference.offscreenLeft, duration: duration)
